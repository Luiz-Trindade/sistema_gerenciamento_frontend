<template>
    <q-page class="q-pa-sm q-pa-md-sm">
                                ...
                                <q-card-section class="q-pa-md">
                                    ...
                                    <div class="row q-col-gutter-sm items-end">
                                        <div class="col-12 col-sm-6">
                                            <!-- ⬇️ AQUI: use-input + filter customizado -->
                                            <q-select v-model="tempItem.produto_id" :options="produtosFiltrados"
                                                use-input hide-selected fill-input input-debounce="0"
                                                @filter="filterProdutos" emit-value map-options outlined dense
                                                label="Produto" :disable="saving">
                                                <template #prepend>
        ...
        <!-- ============================================ -->
        <!-- DIÁLOGO: COMPROVANTE (COMPONENTE)            -->
        <!-- ============================================ -->
        <PedidoReceiptDialog v-model="receiptDialog" :pedido-id="selectedPedidoId" />
    </q-page>
</template>
...
import PedidoReceiptDialog from '@/components/pedidos/PedidoReceiptDialog.vue'
...
// ============================================================
// Filtro de produtos (use-input no q-select)
// ============================================================
// Mantemos uma cópia filtrada. O `filterProdutos` é chamado
// pelo Quasar a cada tecla digitada, com o valor e um `update`
// que devemos chamar passando a nova lista de opções.
// ============================================================
...
const filterProdutos = (val, update) => {
    update(() => {
        ...
        if (!termo) {
            produtosFiltrados.value = produtosOptions.value
            return
        }
        produtosFiltrados.value = produtosOptions.value.filter(opt =>
            opt.label.toLowerCase().includes(termo)
        )
    })
}
